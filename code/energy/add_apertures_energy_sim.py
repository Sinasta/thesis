...
def energy_simulation(Apartment, name):
    ...
    coolingTemp = 25.00
    heatingTemp = 20.00
    energy_model = EnergyModel.EnergyModelByTopology(
    ...
    for i in range(len(EPReportName)):
        value = EnergyModel.EnergyModelQuery(sim_model, EPReportName[i], EPReportForString[i], EPTableName[i], EPColumnName[i], EPRowName[i], EPUnits[i])
